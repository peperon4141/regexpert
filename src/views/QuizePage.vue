...
main.d-flex.flex-row
  .flex-grow-1.mr-auto.border-left.d-flex.flex-column.align-items-start
    section.w-100
      h2 クイズ : {{ current_level.category }} > {{ current_quiz.title }}
        span {{ current_level.rule }}
      b-container(fluid)
        b-col
          b-row
            b-col(cols="12")
  ...
  aside.p-2.border-left.d-none.d-sm-block
    template(v-for="(item, index) in quizzes")
      b-card.mb-1(
        no-body
        bg-variant="light"
      )
        b-card-header.p-1.d-flex.justify-content-between.align-items-center(
          ...
          :class="{'text-primary': is_current_level(item)}"
        )
          b-badge(variant="light" pill) {{ item.list.length }}
          span {{ item.category }}
          b-button.py-0(
            size="sm"
            @click.self="select_level(item, null)"
            variant="secondary"
          ) 選択
        b-collapse(
          ...
          b-list-group(flush)
            b-list-group-item.py-1.pr-1.d-flex.justify-content-between.align-items-center(
              v-for="(quiz, index) in item.list"
              vertical-align="center"
              :class="{'text-primary': is_current_quiz(item, quiz)}"
            )
              span {{ quiz.title }}
              b-button.py-0(size="sm" @click.self="select_level(item, quiz)") 選択
</template>
...
export default {
  ...
  created: function() {
    this.current_level = this.basicLevel
  ...
  methods: {
    ...
    select_level: function(level, quiz) {
      ...
      if (quiz) {
        ...
        this.quiz = this.current_level.list[0]
      }
    },
    is_current_level: function(level) {
      return this.is_equal(this.current_level, level)
    },
    ...
    prev_level: function() {
      let current_level_index = this.quizzes.indexOf(this.current_level)
      ...
      if (0 <= prev_level_index_candidate) {
        ...
        this.current_level = this.quizzes[prev_level_index]
        this.current_quiz = this.current_level.list[this.current_level.list.length - 1]
      }
    },
    prev_quiz: function() {
      let current_quiz_index = this.current_level.list.indexOf(this.current_quiz)
      ...
      if (0 <= prev_quiz_index_candidate) {
        ...
        this.current_quiz = this.current_level.list[target_quiz_index]
      } else {
        this.prev_level()
      }
    },
    next_level: function() {
      ...
      if (next_level_index_candidate <= max_level_index) {
        ...
        this.current_level = this.quizzes[target_level_index]
    ...
    next_quiz: function() {
      ...
      let max_quiz_index = this.current_level.list.length - 1
